...
```bash
git clone https://github.com/fhrochaf/SAR_GSD.git
cd SAR_GSD
```
...
```python
from sar_gsd.download import SentinelDownloader
...
from sar_gsd.deformation import calculate_velocity_map, identify_deformation_hotspots
...
```python
from sar_gsd.visualization import plot_velocity_map, plot_timeseries
...
├── src/
│   └── sar_gsd/          # Main SAR analysis package
...
> **Note:** This implementation uses **SAR intensity** changes, not interferometric phase. For true SAR displacement measurements, extend to SLC processing.
